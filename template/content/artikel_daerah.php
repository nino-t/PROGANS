<?php
	...
	if (isset($_GET['kota'])) {
		$kota = $_GET['kota'];
		$paginateUrl = 'artikel/k/'.$kota;
		$result = $db->query("SELECT * FROM v_article WHERE kota_id = '$kota' AND article_sts = 1 ORDER BY article_id DESC LIMIT $posisi,$batas");
		$alldata = $db->query("SELECT * FROM v_article WHERE kota_id = '$kota' AND article_sts = 1")->num_rows;
	} else if(isset($_GET['provinsi'])){
		$provinsi = $_GET['provinsi'];
		$paginateUrl = 'artikel/p/'.$provinsi;
		$result = $db->query("SELECT * FROM v_article WHERE provinsi_id = '$provinsi' AND article_sts = 1 ORDER BY article_id DESC LIMIT $posisi,$batas");
		$alldata = $db->query("SELECT * FROM v_article WHERE provinsi_id = '$provinsi' AND article_sts = 1")->num_rows;
	}
...
<div id="subheader2" data-type="background" data-speed="3">
    <div id="subheader2-inner" class="inner-block">
        <div class="site-name">Kesini.com</div>
